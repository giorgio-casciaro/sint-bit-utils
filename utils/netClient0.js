  ...
  rpc: async (serviceName, methodName, data, meta, asStream, toEveryTask = false, addedStream = false, removedStream = false) => {
    ...
    try {
      if (toEveryTask && asStream) {
        ...
        while (!conns) {
          hosts = mainWatcher.hosts
          if (hosts) conns = await zeromqClient.getConnectedConnections(hosts.map(hostToPath))
          if (!conns || conns.length !== hosts.length) await new Promise((resolve) => setTimeout(resolve, 100))
        }
        ...
        while (!conn) {
          ...
          if (!conn) await new Promise((resolve) => setTimeout(resolve, 100))
        }
